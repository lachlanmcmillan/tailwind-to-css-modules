...
// assume the third argument is a path
...
async function transform(filePath: string) {
  ...
  if (counter > 0) {
    const cssModulePath = filePath.substring(0, filePath.length - 3) + "module.css"
    const cssModuleRelativePath = cssModulePath.split('/').findLast(x => true);
